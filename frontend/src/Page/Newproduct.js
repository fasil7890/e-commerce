...
const Newproduct = () => {
  ...
  return (
    <div className='p-4'>
      <form className='m-auto w-full max-w-md  shadow flex flex-col p-3 bg-white' onSubmit={handleSubmit}>
        ...
        <div className='h-40 w-full bg-slate-200 my-3 rounded flex items-center justify-center cursor-pointer'>
          {
            data.image ? <img src={data.image} className="h-full "  alt="img" /> : <span className='text-5xl'><BsCloudUpload/></span>
          }
   <input type={"file"} accept="image/*" id='image' onChange={uploadImage} className="hidden"  />
        ...
        <textarea value={data.description} className='bg-slate-200 p-1 my-1 resize-none' name="description" rows={2} onChange={handleOnChange}></textarea>
