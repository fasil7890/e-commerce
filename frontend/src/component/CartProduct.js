...
const CartProduct = ({id,name,image,category,qty,total,price}) => {
  ...
  return (
    <div className='bg-slate-200 p-2 flex gap-4 rounded border m-1 border-slate-400'>
        <div className='p-3 bg-white rounded overflow-hidden'>
             <img src={image} alt="img" className="h-28 w-40 object-cover" />
        </div>
       ...
       <AiFillDelete/>
        ...
        <button onClick={()=>dispatch(increaseQty(id))} className='bg-slate-300 py-1 mt-2 rounded hover:bg-slate-200 p-1'><TbPlus/></button>
        <p className='font-semibold p-1'>{qty}</p>
        <button onClick={()=>dispatch(decreaseQty(id))} className='bg-slate-300 py-1 mt-2 rounded hover:bg-slate-200 p-1'><TbMinus/></button>
        </div>
